...
impl BlockchainRetryWorker {
    ...
    pub async fn process_batch(&self) {
        match self
            .service
            .process_pending_wallet_approvals(self.config.batch_size)
            .await
        {
            Ok(0) => debug!("No pending wallet approvals found"),
            Ok(count) => info!(count = count, "Processed pending wallet approvals"),
            Err(e) => error!(error = ?e, "Error processing wallet approvals"),
        }
...
mod tests {
    use super::*;
    use crate::domain::{
        BlockchainStatus, ComplianceDecision, ComplianceLevel, ComplianceStatus, DatabaseClient,
    ...
    async fn test_run_once_processes_pending_items() {
        ...
        let request = SubmitTransferRequest {
            ...
            token_mint: Some("MintAddress11111111111111111111111111111111".to_string()),
        ...
        db.update_compliance_status(&tr.id, ComplianceStatus::Approved)
            .await
            .unwrap();
        db.enqueue_wallet_approval_if_missing(
