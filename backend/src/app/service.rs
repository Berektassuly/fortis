...
impl AppService {
    ...
    /// 4. Screen the receiver wallet and enqueue wallet approval
    /// 5. Queue the public token transfer for worker submission
    ///
    ...
    pub async fn submit_transfer(
        ...
        let decision = self
            .compliance_provider
            .screen_wallet(&request.to_address)
            .await?;

        if decision.status == ComplianceStatus::Rejected {
            ...
            let rejection_reason = decision
                .reasoning
                .clone()
                .unwrap_or_else(|| "Range Protocol: High-risk address detected".to_string());
                ...
                && blocklist.check_address(&request.to_address).is_none()
            {
                let risk_level = decision
                    .risk_level
                    .clone()
                    .unwrap_or_else(|| "High risk".to_string());
                info!(
                    address = %request.to_address,
                ...
                let _ = blocklist
                    .add_address(
                        ...
                        format!("Auto-blocked: Range Protocol {}", risk_level),
                    )
                    .await;
            }
        ...
        // =====================================================================
        // STEP 4: Enqueue wallet approval and queue transfer submission
        // =====================================================================
        let token_mint = request.required_token_mint()?;
        let wallet_approval = self
            .db_client
            .enqueue_wallet_approval_if_missing(token_mint, &request.to_address, &decision)
            .await?;

        self.db_client
            .mark_transfer_approved(&request_id, &decision, &wallet_approval)
            .await?;
        transfer_request.compliance_status = ComplianceStatus::Approved;
        transfer_request.asset_record_pda = Some(wallet_approval.asset_record_pda.clone());
        transfer_request.receiver_compliance_pda =
            Some(wallet_approval.compliance_record_pda.clone());
        transfer_request.range_risk_score = decision.risk_score;
        ...
        info!(
            ...
            wallet_approval_id = %wallet_approval.id,
            compliance_status = %transfer_request.compliance_status.as_str(),
            blockchain_status = %transfer_request.blockchain_status.as_str(),
            "Transfer approved, wallet approval queued, and transfer scheduled for worker submission"
        );
    ...
    /// Process pending wallet approvals before transfer relay.
    #[instrument(skip(self))]
    pub async fn process_pending_wallet_approvals(
        &self,
        batch_size: i64,
    ) -> Result<usize, AppError> {
        let pending_approvals = self
            .db_client
            .get_pending_wallet_approvals(batch_size)
            .await?;
        let count = pending_approvals.len();

        if count == 0 {
            return Ok(0);
        }

        info!(count = count, "Processing pending wallet approvals");

        for approval in pending_approvals {
            if let Err(e) = self.process_single_wallet_approval(&approval).await {
                error!(
                    approval_id = %approval.id,
                    wallet = %approval.wallet_address,
                    error = ?e,
                    "Failed to process wallet approval"
                );
            }
        }
            ...
            Ok(submission) => {
                self.db_client
                    .update_wallet_approval_status(
                        ...
                        submission.tx_signature.as_deref(),
                        None,
                        None,
                        Some(Utc::now()),
                    )
                ...
                info!(
                    ...
                    signature = ?submission.tx_signature,
                    "Wallet approval confirmed and recorded"
                );
            }
            Err(e) => {
