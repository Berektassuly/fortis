...
use crate::domain::{AppError, ComplianceDecision, ComplianceLevel, ComplianceProvider};
...
impl RangeComplianceProvider {
    ...
    fn mock_check(&self, wallet_address: &str) -> ComplianceDecision {
        // Block strict match for known test addresses
        if wallet_address == "hack_the_planet_bad_wallet" {
            return ComplianceDecision::rejected(
                Some(10),
                Some("Critical risk".to_string()),
                Some("Mock blocklist hit".to_string()),
            );
        }
        ...
        if wallet_address.to_lowercase().starts_with("hack") {
            return ComplianceDecision::rejected(
                Some(10),
                Some("Critical risk".to_string()),
                Some("Mock heuristic rejected wallet".to_string()),
            );
        }
    ...
    fn compliance_level_for_score(&self, risk_score: i32) -> ComplianceLevel {
        match risk_score {
            i32::MIN..=2 => ComplianceLevel::Basic,
            3..=5 => ComplianceLevel::Standard,
            _ => ComplianceLevel::Enhanced,
        }
    }

    fn evaluate_risk(&self, response: &RiskResponse) -> ComplianceDecision {
        ...
        if is_high_risk {
            ...
            ComplianceDecision::rejected(
                ...
                Some(response.risk_level.clone()),
            ...
            ComplianceDecision::approved(
                self.compliance_level_for_score(response.risk_score),
                Some(response.risk_score),
                Some(response.risk_level.clone()),
                Some(response.reasoning.clone()),
            )
...
impl ComplianceProvider for RangeComplianceProvider {
    ...
    async fn screen_wallet(&self, wallet_address: &str) -> Result<ComplianceDecision, AppError> {
        ...
        if self.is_mock_mode() {
            ...
            return Ok(self.mock_check(wallet_address));
        }

        match self.check_address_risk(wallet_address).await {
            ...
            Err(e) => {
                ...
                Ok(ComplianceDecision::rejected(
                    ...
                    Some("Range Protocol API error".to_string()),
                ))
            }
        }
    }
...
mod tests {
    ...
    fn test_mock_mode_approved() {
        ...
        let decision = provider.mock_check("HvwC9QSAzwEXkUkwqNNGhfNHoVqXJYfPvPZfQvJmHWcF");
        assert_eq!(decision.status, ComplianceStatus::Approved);
        assert_eq!(decision.level, ComplianceLevel::Standard);
    }

    #[test]
    fn test_mock_mode_rejected_exact_match() {
        let provider = RangeComplianceProvider::new(None, None, None);
        let decision = provider.mock_check("hack_the_planet_bad_wallet");
        assert_eq!(decision.status, ComplianceStatus::Rejected);
    }

    #[test]
    fn test_mock_mode_rejected_prefix() {
        let provider = RangeComplianceProvider::new(None, None, None);
        let decision = provider.mock_check("hackSomeAddress123");
        assert_eq!(decision.status, ComplianceStatus::Rejected);
    }
    ...
    fn test_risk_evaluation_medium_risk_approved() {
        ...
        let decision = provider.evaluate_risk(&response);
        assert_eq!(decision.status, ComplianceStatus::Approved);
        assert_eq!(decision.level, ComplianceLevel::Standard);
    }
    ...
    fn test_risk_evaluation_low_risk_approved() {
        ...
        let decision = provider.evaluate_risk(&response);
        assert_eq!(decision.status, ComplianceStatus::Approved);
        assert_eq!(decision.level, ComplianceLevel::Basic);
    }

    #[tokio::test]
    async fn test_screen_wallet_mock_mode_approved() {
        let provider = RangeComplianceProvider::new(None, None, None);
        let result = provider.screen_wallet("receiver").await.unwrap();
        assert_eq!(result.status, ComplianceStatus::Approved);
    }

    #[tokio::test]
    async fn test_screen_wallet_mock_mode_rejected() {
        let provider = RangeComplianceProvider::new(None, None, None);
        let result = provider.screen_wallet("hackBadWallet").await.unwrap();
    ...
    fn test_relaxed_threshold_allows_high_risk() {
        ...
        assert_eq!(
            provider.evaluate_risk(&high_risk_response).status,
        ...
        assert_eq!(
            provider.evaluate_risk(&extremely_high_response).status,
        ...
        assert_eq!(
            provider.evaluate_risk(&critical_response).status,
            ComplianceStatus::Rejected
        );
    }
}
